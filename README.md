...
Skill 直接调用可用的原生生图工具，先生成右侧科学主体与左侧自然留白背景，再用固定版式叠加标题。结果应包含封面图，而不是停留在提示词阶段。若宿主只能生图、不能后期排版，Skill 仍直接生图，但会改用含准确标题的完整提示词，并说明中文渲染可能需要修正。
...
- 中文默认字体：`YangRendongZhushi-Light.ttf`
