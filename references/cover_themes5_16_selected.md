...
- 准确标题由后期排版叠加；先锁定标题区、图像排除区、字体语气与融合装置，再生成底图。quiet zone 必须服从主题构图，不得让所有主题固定成左文右图。
- 禁止把标题放进通用半透明矩形、深色信息卡或 app UI 样式容器。标题应直接落在主题原生的材质或版式结构中。
- 示例图只约束媒介、构图、色板和层级；不得复制示例中的乱码、错误结构或平台星形标记。
- 每个最终 Prompt 必须加入本文件末尾的通用 Negative prompt。

## 标题优先版式系统（Theme 5–16）

所有坐标均为画布比例。主标题最多 2–3 行，副标题最多 1 行；图像生成阶段必须排空对应区域，最终由后期准确排入中文。

| Theme | 固定标题区 | 图像排除与融合装置 | 字体语气与颜色 |
| --- | --- | --- | --- |
| 5 Nature | 左上 `x=7%-39%, y=13%-45%` | 午夜蓝自然暗场，主体避开；一条细冰青基线 | 冷白高对比无衬线，珊瑚红作短小强调 |
| 6 Businessweek | 左侧 `x=6%-43%, y=18%-72%` | 酸性黄实底本身即版面，巨物在右侧形成压迫 | 超粗黑体主标题，临床红小副标，强硬左对齐 |
| 7 Monocle | 上方 `x=8%-48%, y=8%-30%` | 象牙色建筑天空/立面留白，细建筑分隔线 | 森林绿中粗衬线或人文无衬线，砖红小色标 |
| 8 Micro | 右上 `x=61%-92%, y=12%-45%` | 失焦深海蓝液相区，主结合点不得侵入 | 银白窄体无衬线，荧光绿细线强调 |
| 9 Swiss | 左侧 `x=7%-49%, y=15%-84%` | 纸白网格是标题底座，几何主体围绕文字排布 | 超粗黑体，钴蓝与信号红作为单字/规则强调 |
| 10 Archive | 左上 `x=8%-41%, y=12%-47%` | 未印刷旧纸字段，周围可有裁切线与无字批注 | 炭黑压印感衬线或打字机感字体，氧化红档案编号点 |
| 11 Pipeline | 中上横带 `x=13%-87%, y=10%-28%` | 暖灰交通图标题带，路线在带上下分叉，不可穿越文字 | 海军蓝中粗无衬线，青绿细路线作为下划线 |
| 12 Clinical | 顶部刊头 `x=7%-54%, y=8%-25%` | 临床白刊头区，证据层从下方开始 | 深海军蓝理性无衬线，安全橙仅用于短标记 |
| 13 Cell | 顶部 `x=7%-50%, y=8%-27%` | 暖白显微留白，细胞剖面从 `y=30%` 开始 | 海军蓝出版级无衬线，RNA 金细规则 |
| 14 Congress | 中央 `x=22%-78%, y=34%-62%` | 粒子密度主动降低，留下自然光雾窗口 | 冷白高字重无衬线，品红与医学青短线点缀 |
| 15 Blueprint | 顶部中轴 `x=18%-82%, y=8%-25%` | 冰白标题字段与图纸网格连续，测量线绕开文字 | 普鲁士蓝工程字形，结构青规则、节点黄定位点 |
| 16 Bioprocess | 右侧竖栏 `x=70%-93%, y=16%-76%` | 洁净白设备铭牌/墙面，管道不进入文字区 | 深青工业无衬线，琥珀色细分隔线 |
